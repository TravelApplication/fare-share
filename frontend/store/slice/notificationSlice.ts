...
export type NotificationType = 'VOTE' | 'VOTE_CHANGE' | 'FRIEND_INVITATION' | 'GROUP_INVITATION' | 'FRIEND_INVITATION_ACCEPT' | 'FRIEND_INVITATION_REJECT' | 'GROUP_INVITATION_ACCEPT' | 'GROUP_INVITATION_REJECT';
...
export const createNotificationSlice: StateCreator<NotificationSlice, [], [], NotificationSlice> = (set) => ({
    ...
    setToFetchFriendInvitations: (value) => set({ toFetchFriendInvitations: value }),
    setToFetchGroupInvitations: (value) => set({ toFetchGroupInvitations: value }),
});
